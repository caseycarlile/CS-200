...
void Program::GameLoop()
{
	while(CheckForWinner() == -1)
	{
		...
		bool canPlaceMarker = false;

		int x;
		int y;

		while (canPlaceMarker == false)
		{
			cout << "Player " << turn << " , enter an X coordinate.";
			x = GetValidInput(0, 2);
			cout << "Player " << turn << " , enter a Y coordinate.";
			y = GetValidInput(0, 2);
			
			canPlaceMarker = PlaceMarker(x, y, playerMarker[turn]);
		}
...
int Program::GetValidInput( int min, int max )
{
	...
	while (valid == false)
	{
		...
		{
			cout << "Your choice is invalid. Please enter a different coordinate." << endl;
		}
	}
}
